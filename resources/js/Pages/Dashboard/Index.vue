...
const showBalance = ref(localStorage.getItem('showBalance') !== 'false');
watch(showBalance, (val) => localStorage.setItem('showBalance', String(val)));
        ...
        <div class="mb-8 pt-2">
            <div class="flex flex-col items-center justify-center text-center">
                <p class="text-sm font-medium text-content-muted mb-1 flex items-center gap-2">
                    Tổng số dư
                    <button type="button" class="text-content-muted hover:text-content transition-colors" @click="showBalance = !showBalance">
                        <svg v-if="showBalance" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                ...
                <h1 class="text-4xl sm:text-5xl font-bold text-content tracking-tight">
                    <span v-if="showBalance">{{ formatMoney(totalBalance, false) }} <span class="text-2xl opacity-70">₫</span></span>
                    <span v-else>******</span>
                </h1>
            </div>
