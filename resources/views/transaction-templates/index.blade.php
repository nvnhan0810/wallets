...
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-900">Mẫu giao dịch</h2>
    <p class="mt-1 text-sm text-gray-500">Thu, chi, cân đối số dư hoặc chuyển/rút giữa các ví.</p>
</div>

<div class="bg-white shadow rounded-lg p-6 mb-8" x-data="{ type: 'expense' }">
    ...
    <form action="{{ route('transaction-templates.store') }}" method="POST" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700">Tên mẫu</label>
            <input type="text" name="name" required class="mt-1 w-full rounded-md border border-gray-300 p-2">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Loại</label>
            <select name="type" x-model="type" required class="mt-1 w-full rounded-md border border-gray-300 p-2">
        ...
        <div>
            ...
            <input type="text" name="description" class="mt-1 w-full rounded-md border border-gray-300 p-2">
        </div>

        <div x-show="type === 'income' || type === 'expense'" class="contents">
            <div>
                <label class="block text-sm font-medium text-gray-700">Danh mục</label>
                <input type="text" name="category" :disabled="type !== 'income' && type !== 'expense'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Ví mặc định</label>
                <select name="default_wallet_id" :disabled="type !== 'income' && type !== 'expense'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
                    <option value="">—</option>
        ...
        <div x-show="type === 'adjustment'" x-cloak class="contents">
            <div>
                ...
                <select name="default_wallet_id" :disabled="type !== 'adjustment'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
                    @foreach($wallets as $w)
                        <option value="{{ $w->id }}">{{ $w->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="adjustment_direction" :disabled="type !== 'adjustment'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
        ...
        <div x-show="type === 'transfer'" x-cloak class="contents">
            <div>
                ...
                <select name="from_wallet_id" :disabled="type !== 'transfer'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
                    @foreach($wallets as $w)
                        <option value="{{ $w->id }}">{{ $w->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="to_wallet_id" :disabled="type !== 'transfer'" class="mt-1 w-full rounded-md border border-gray-300 p-2">
                    @foreach($wallets as $w)
                        <option value="{{ $w->id }}">{{ $w->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="sm:col-span-2 lg:col-span-3">
            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">Thêm mẫu</button>
        </div>
    </form>
</div>
...
<div class="bg-white shadow rounded-lg divide-y divide-gray-100">
    ...
    <div class="px-4 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div class="flex gap-3 text-sm">
            <a href="{{ route('transactions.create', ['template_id' => $t->id]) }}" class="text-indigo-600 hover:underline">Dùng mẫu</a>
    ...
    @empty
    <p class="px-4 py-8 text-center text-gray-500">Chưa có mẫu. Tạo ở đây hoặc lưu từ form giao dịch.</p>
    @endforelse
</div>
@endsection
